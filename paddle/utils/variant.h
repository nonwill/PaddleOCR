...
private:
  ...
  template <typename Visitor> struct PYBIND11_HIDDEN value_visitor {
    Visitor &&visitor_;
...
template <std::size_t I, typename T> struct overload_leaf {
  using F = lib::size_constant<I> (*)(T);
  operator F() const { return nullptr; }
};

template <typename... Ts> struct overload_impl {
private:
  template <typename> struct impl;
...
template <typename... Ts> using overload = typename overload_impl<Ts...>::type;
...
template <> struct hash<paddle::monostate> {
