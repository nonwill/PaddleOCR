 ...
 *
 * Define CV_LOG_STRIP_LEVEL=CV_LOG_LEVEL_[DEBUG|INFO|WARN|ERROR|FATAL|DISABLED]
...
#define CV_LOG_WITH_TAG(tag, msgLevel, extra_check0, extra_check1, msg_prefix, \
  ...
  for (;;) {                                                                   \
    extra_check0;                                                              \
    if (cv::utils::logging::getLogLevel() < msgLevel)                          \
      break;                                                                   \
    extra_check1;                                                              \
    std::stringstream ss;                                                      \
    ss msg_prefix << __VA_ARGS__;                                              \
...
#define CV_LOG_VERBOSE(tag, v, ...)                                            \
  CV_LOG_WITH_TAG(tag, cv::utils::logging::LOG_LEVEL_VERBOSE, , ,              \
                  << "[VERB" << v << ":" << cv::utils::getThreadID() << "] ",  \
                  __VA_ARGS__)
