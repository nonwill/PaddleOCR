...
CV_EXPORTS const SVMCapabilities
getSVMCapabilitites(const ocl::Context &context);
...
struct SVMFunctions {
  ...
  inline SVMFunctions()
      : fn_clSVMAlloc(NULL), fn_clSVMFree(NULL),
        ...
        fn_clEnqueueSVMMemFill(NULL), fn_clEnqueueSVMMap(NULL),
        fn_clEnqueueSVMUnmap(NULL) {
    // nothing
  }

  inline bool isValid() const {
    return fn_clSVMAlloc != NULL && fn_clSVMFree &&
           fn_clSetKernelArgSVMPointer &&
           /*fn_clSetKernelExecInfo && fn_clEnqueueSVMFree &&*/
           fn_clEnqueueSVMMemcpy && fn_clEnqueueSVMMemFill &&
           fn_clEnqueueSVMMap && fn_clEnqueueSVMUnmap;
  }
};

// We should guarantee that SVMFunctions lifetime is not less than context's
// lifetime
