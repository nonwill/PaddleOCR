...
struct error : public std::exception {
  ...
  static void wrap_c_api(dnnl_status_t status, const char *message) {
    if (status != dnnl_success)
      DNNL_THROW_ERROR(status, message);
  }
};
...
///
template <typename T, typename traits = handle_traits<T>> struct handle {
private:
...
public:
  ...
  T get(bool allow_empty = false) const {
    ...
    if (allow_empty == false && result == nullptr)
      DNNL_THROW_ERROR(dnnl_invalid_arguments, "object is not initialized");
    return result;
  }
...
/// @cond DO_NOT_DOCUMENT_THIS
template <> struct handle_traits<dnnl_engine_t> {
...
struct engine : public handle<dnnl_engine_t> {
  ...
  engine(kind akind, size_t index) {
    ...
    error::wrap_c_api(dnnl_engine_create(&engine, convert_to_c(akind), index),
...
/// @cond DO_NOT_DOCUMENT_THIS
template <> struct handle_traits<dnnl_stream_t> {
...
struct stream : public handle<dnnl_stream_t> {
  ...
  explicit stream(const engine &aengine, flags aflags = flags::default_flags) {
    dnnl_stream_t stream;
    error::wrap_c_api(
        dnnl_stream_create(&stream, aengine.get(),
  ...
  stream &wait() {
    error::wrap_c_api(dnnl_stream_wait(get()), "could not wait on a stream");
    return *this;
  }
};
...
#define DNNL_DEFINE_BITMASK_OPS(enum_name)                                     \
  inline enum_name operator|(enum_name lhs, enum_name rhs) {                   \
    return static_cast<enum_name>(static_cast<unsigned>(lhs) |                 \
  ...
  inline enum_name operator^(enum_name lhs, enum_name rhs) {                   \
    return static_cast<enum_name>(static_cast<unsigned>(lhs) ^                 \
                                  static_cast<unsigned>(rhs));                 \
                                                                               ...
                                                                               \
  inline enum_name &operator|=(enum_name &lhs, enum_name rhs) {                \
    lhs = static_cast<enum_name>(static_cast<unsigned>(lhs) |                  \
                                 static_cast<unsigned>(rhs));                  \
    return lhs;                                                                \
  }                                                                            \
                                                                               \
