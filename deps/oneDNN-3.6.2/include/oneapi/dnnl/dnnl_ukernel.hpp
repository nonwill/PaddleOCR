...
struct attr_params : public handle<dnnl_ukernel_attr_params_t> {
  ...
  attr_params() {
    ...
    error::wrap_c_api(status, "could not create an attributes memory storage");
    reset(c_params);
  }
  ...
  void set_post_ops_args(const void **post_ops_args) {
    dnnl_status_t status =
        dnnl_ukernel_attr_params_set_post_ops_args(get(), post_ops_args);
    if (status != dnnl_success)
      error::wrap_c_api(status, "could not set post operations arguments");
  }
...
struct brgemm : public handle<dnnl_brgemm_t> {
  ...
  brgemm(memory::dim M, memory::dim N, memory::dim K, memory::dim batch_size,
         ...
         memory::data_type a_dt, memory::data_type b_dt, memory::data_type c_dt,
         bool allow_empty = false) {

    dnnl_brgemm_t brgemm = nullptr;
    dnnl_status_t status = dnnl_brgemm_create(
        &brgemm, M, N, K, batch_size, lda, ldb, ldc, memory::convert_to_c(a_dt),
        memory::convert_to_c(b_dt), memory::convert_to_c(c_dt));

    if (!allow_empty)
      error::wrap_c_api(status, "could not create a BRGeMM ukernel object");
    reset(brgemm);
  }
  ...
  size_t get_scratchpad_size() const {
    ...
    if (status != dnnl_success)
      error::wrap_c_api(
          status, "could not query a scratchpad size from a BRGeMM ukernel "
                  "object");
    return size;
  }
  ...
  void
  execute(const void *A, const void *B,
    ...
    dnnl_status_t status = dnnl_brgemm_execute(
        get(), A, B, (const dnnl_dim_t *)A_B_offsets.data(), C, scratchpad);
    if (status != dnnl_success)
      error::wrap_c_api(status, "could not execute a BRGeMM ukernel object");
  }
  ...
  execute(const void *A, const void *B,
    ...
    // TODO: export batch_element to C API later for user to fill it and
    // pass directly to the call.
    dnnl_status_t status = dnnl_brgemm_execute_postops(
        get(), A, B, (const dnnl_dim_t *)A_B_offsets.data(), C, D, scratchpad,
        params.get());
    if (status != dnnl_success)
      error::wrap_c_api(status, "could not execute a BRGeMM ukernel object");
  }
...
struct transform : public handle<dnnl_transform_t> {
  ...
  transform(memory::dim K, memory::dim N, pack_type in_pack_type,
    ...
    dnnl_status_t status = dnnl_transform_create(
        &transform, K, N, static_cast<dnnl_pack_type_t>(in_pack_type), in_ld,
        out_ld, memory::convert_to_c(in_dt), memory::convert_to_c(out_dt));
