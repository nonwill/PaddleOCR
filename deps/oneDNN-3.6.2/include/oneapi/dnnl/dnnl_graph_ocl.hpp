...
/// @returns Created engine
inline engine
make_engine_with_allocator(cl_device_id device, cl_context context,
                           const allocator &alloc,
...
inline cl_event execute(compiled_partition &c_partition, stream &astream,
  ...
  error::wrap_c_api(
      dnnl_graph_ocl_interop_compiled_partition_execute(
          c_partition.get(), astream.get(), c_inputs.size(), c_inputs.data(),
          c_outputs.size(), c_outputs.data(), c_deps, (int)deps.size(),
          &ocl_event),
