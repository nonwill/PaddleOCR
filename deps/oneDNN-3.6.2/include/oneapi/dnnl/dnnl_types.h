...
/// `(batch, channels, depth, height, width)`, while the physical layout might
/// be `NCDHW` (corresponds to #dnnl_ncdhw format tag) or `NDHWC` (corresponds
/// to #dnnl_ndhwc format tag).
///
/// ~~~cpp
...
/// A special mnemonic for RNN input recurrent cell attention vector. An alias
/// for #DNNL_ARG_SRC_3.
...
/// Single seed needed independently of how many arguments need stochastic
/// rounding
...
///     during the lifetime of the queried primitive descriptor. Returned
///     objects must not be destroyed by the user. If you need to keep the
///     object longer than the lifetime of the queried primitive descriptor, use
