...
dnnl::primitive::kind primitive::get_kind() const {
  ...
  error::wrap_c_api(
      dnnl_primitive_desc_query(pd, dnnl_query_primitive_kind, 0,
                                (void *)&kind),
...
struct memory : public handle<dnnl_memory_t> {
  ...
  static void validate_dims(const std::vector<T> &v, int min_size = 0) {
    validate_container_size(v, "dimensions are invalid", min_size,
                            DNNL_MAX_NDIMS);
  }
  ...
  enum class data_type {
    ...
    f16 = dnnl_f16,
    /// non-standard
    /// [16-bit floating point with 7-bit
    /// mantissa](https://en.wikipedia.org/wiki/Bfloat16_floating-point_format).
    bf16 = dnnl_bf16,
  ...
  enum class format_tag {
    ...
    /// 4D CNN weights tensor with groups; an alias for
    ...
    wigo = dcab,
    /// 5D CNN weights tensor with groups; an alias for
    ...
    giohw = acbde,
    /// 6D CNN weights tensor with groups; an alias for
  ...
  struct desc : public handle<dnnl_memory_desc_t> {
    ...
    desc() {
      ...
      error::wrap_c_api(dnnl_memory_desc_create_with_tag(&zero_md, 0, nullptr,
    ...
    desc(const dims &adims, data_type adata_type, format_tag aformat_tag,
      ...
      dnnl_status_t status = dnnl_memory_desc_create_with_tag(
          &md, (int)adims.size(), adims.data(), convert_to_c(adata_type),
    ...
    static desc csr(const dims &adims, data_type adata_type, dim nnz,
      ...
      dnnl_status_t status = dnnl_memory_desc_create_with_csr_encoding(
          &md, (int)adims.size(), adims.data(), convert_to_c(adata_type), nnz,
          convert_to_c(index_dt), convert_to_c(pointer_dt));
      if (!allow_empty)
        error::wrap_c_api(status,
    ...
    desc(const std::vector<uint8_t> &blob) {
      dnnl_memory_desc_t md = nullptr;
      error::wrap_c_api(dnnl_memory_desc_create_with_blob(&md, blob.data()),
    ...
    desc reshape(const dims &adims, bool allow_empty = false) const {
      ...
      if (!allow_empty)
        error::wrap_c_api(status, "could not reshape a memory descriptor");
      return desc(out_md);
    }
    ...
    desc permute_axes(const std::vector<int> &permutation,
      ...
      dnnl_status_t status =
          dnnl_memory_desc_permute_axes(&out_md, get(), permutation.data());
      if (!allow_empty)
        error::wrap_c_api(status,
    ...
    int get_inner_nblks() const { return query_s32(query::inner_nblks_s32); }
    ...
    dim get_nnz() const {
      dnnl_dim_t nnz;
      dnnl_status_t status =
          dnnl_memory_desc_query_v2(get(), dnnl_query_nnz_s64, 0, &nnz);
    ...
    memory::sparse_encoding get_sparse_encoding() const {
      ...
      return status == dnnl_success
                 ? static_cast<dnnl::memory::sparse_encoding>(sparse_encoding)
    ...
    memory::format_kind get_format_kind() const {
      ...
      dnnl_status_t status =
          dnnl_memory_desc_query(get(), dnnl_query_format_kind, &format_kind);
      return status == dnnl_success
    ...
    /// @returns The memory descriptor blob associated with the memory
    /// descriptor
    std::vector<uint8_t> get_blob() {
      size_t size;
      dnnl_status_t status = dnnl_memory_desc_get_blob(nullptr, &size, get());
      error::wrap_c_api(status, "could not get memory descriptor blob size");
  ...
  private:
    ...
    memory::data_type query_data_type(query what) const {
      dnnl_data_type_t data_type;
      dnnl_status_t status =
          dnnl_memory_desc_query(get(), dnnl::convert_to_c(what), &data_type);
      return status == dnnl_success
                 ? static_cast<dnnl::memory::data_type>(data_type)
                 : dnnl::memory::data_type::undef;
    }
    ...
    int query_s32(query what) const {
      int res;
      dnnl_status_t status =
          dnnl_memory_desc_query(get(), dnnl::convert_to_c(what), &res);
    ...
    memory::dims query_dims(query what) const {
      dnnl_dims_t *c_dims;
      dnnl_status_t status =
          dnnl_memory_desc_query(get(), dnnl::convert_to_c(what), &c_dims);

      const int ndims = (what == query::inner_idxs || what == query::inner_blks)
                            ? get_inner_nblks()
                            : get_ndims();

      return status == dnnl_success ? memory::dims(*c_dims, *c_dims + ndims)
                                    : memory::dims{};
    }
  };
  ...
  memory(const desc &md, const engine &aengine, std::vector<void *> handles) {
    ...
    dnnl_status_t status = dnnl_memory_create_v2(
        &result, md.get(), aengine.get(), (int)handles.size(), handles.data());
  ...
  template <typename T = void> T *map_data(int index = 0) const {
    void *mapped_ptr;
...
/// @cond DO_NOT_DOCUMENT_THIS
template <> struct handle_traits<dnnl_post_ops_t> {
...
struct post_ops : public handle<dnnl_post_ops_t> {
  ...
  primitive::kind kind(int index) const {
    ...
    return static_cast<primitive::kind>(dnnl_post_ops_get_kind(get(), index));
  }
  ...
  void get_params_sum(int index, float &scale) const {
    error::wrap_c_api(
        dnnl_post_ops_get_params_sum(get(), index, &scale, nullptr, nullptr),
  ...
  void get_params_sum(int index, float &scale,
  ...
  void get_params_eltwise(int index, algorithm &aalgorithm, float &alpha,
                          float &beta) const {
    dnnl_alg_kind_t c_alg;
    error::wrap_c_api(
        dnnl_post_ops_get_params_eltwise(get(), index, &c_alg, &alpha, &beta),
  ...
  void append_dw(memory::data_type weights_data_type,
                 ...
                 memory::data_type dst_data_type, memory::dim kernel_size,
                 memory::dim stride_size, memory::dim padding_l_size) {

    error::wrap_c_api(
        dnnl_post_ops_append_dw(get(), memory::convert_to_c(weights_data_type),
  ...
  void get_params_dw(int index, memory::data_type &weights_data_type,
    ...
    error::wrap_c_api(dnnl_post_ops_get_params_dw(
                          get(), index, &c_weights_data_type, &c_bias_data_type,
                          &c_dst_data_type, &c_kernel_size, &c_stride_size,
                          &c_padding_l_size),
  ...
  void append_binary(algorithm aalgorithm, const memory::desc &src1_desc) {
    error::wrap_c_api(dnnl_post_ops_append_binary(
                          get(), convert_to_c(aalgorithm), src1_desc.get()),
  ...
  ///
  ///     conv_args.insert(
  ///      {DNNL_ARG_ATTR_MULTIPLE_POST_OP(0) | DNNL_ARG_WEIGHTS,
  ///      prelu_weights})
  /// @endcode
  ///
  /// @note
...
/// @cond DO_NOT_DOCUMENT_THIS
template <> struct handle_traits<dnnl_primitive_attr_t> {
...
struct primitive_attr : public handle<dnnl_primitive_attr_t> {
  ...
  void get_fpmath_mode(fpmath_mode &mode, bool &apply_to_int) const {
    ...
    int c_apply_to_int;
    error::wrap_c_api(
        dnnl_primitive_attr_get_fpmath_mode_v2(get(), &c_mode, &c_apply_to_int),
        "could not get fpmath mode primitive attribute");
  ...
  void set_fpmath_mode(fpmath_mode mode, bool apply_to_int = false) {
    error::wrap_c_api(dnnl_primitive_attr_set_fpmath_mode_v2(
                          get(), dnnl::convert_to_c(mode), apply_to_int),
  ...
  accumulation_mode get_accumulation_mode() const {
    ...
    error::wrap_c_api(dnnl_primitive_attr_get_accumulation_mode(get(), &result),
  ...
  void set_deterministic(bool value) {
    error::wrap_c_api(
        dnnl_primitive_attr_set_deterministic(get(), static_cast<int>(value)),
  ...
  void set_rounding_mode(int arg, rounding_mode mode) {
    error::wrap_c_api(
        dnnl_primitive_attr_set_rounding(get(), arg, convert_to_c(mode)),
  ...
  scratchpad_mode get_scratchpad_mode() const {
    ...
    error::wrap_c_api(dnnl_primitive_attr_get_scratchpad_mode(get(), &result),
  ...
  ///     factors used for that logical dimension in a memory indicated by @p
  ///     arg.
  /// @param data_type Scaling factors data_type.
  void set_scales(int arg, int mask, const memory::dims &groups,
                  memory::data_type data_type = memory::data_type::f32) {
    error::wrap_c_api(dnnl_primitive_attr_set_scales(
                          get(), arg, mask, (int)groups.size(), groups.data(),
                          memory::convert_to_c(data_type)),
                      "could not set scales primitive attribute");
  }
  ...
  ///     correspondence between the tensor dimensions and the zero_points
  ///     array. The set i-th dimension indicates a number of groups of zero
  ///     point factors used for that logical dimension in a memory indicated by
  ///     @p arg.
  ...
  void set_zero_points(int arg, int mask, const memory::dims &groups,
    ...
    error::wrap_c_api(dnnl_primitive_attr_set_zero_points(
                          get(), arg, mask, (int)groups.size(), groups.data(),
                          memory::convert_to_c(data_type)),
                      "could not set zero points primitive attribute");
  }
  ...
  void get_rnn_data_qparams(float &scale, float &shift) {
    ...
    error::wrap_c_api(
        dnnl_primitive_attr_get_rnn_data_qparams(get(), &c_scale, &c_shift),
        "could not set RNN data quantization parameters primitive "
        "attribute");
    scale = c_scale;
    shift = c_shift;
  ...
  void set_rnn_weights_qparams(int mask, const std::vector<float> &scales) {
    error::wrap_c_api(
        dnnl_primitive_attr_set_rnn_weights_qparams(get(), (int)scales.size(),
                                                    mask, scales.data()),
  ...
  ///     Violations can only be detected when the attributes are used to
  ///     create a primitive descriptor.
  void set_rnn_weights_projection_qparams(int mask,
  ...
  ///     Violations can only be detected when the attributes are used to
  ///     create a primitive descriptor.
  void get_rnn_weights_projection_qparams(int &mask,
...
struct primitive_desc_base : public handle<dnnl_primitive_desc_t> {
  ...
  const char *impl_info_str() const {
    const char *res;
    error::wrap_c_api(
        dnnl_primitive_desc_query(get(), dnnl_query_impl_info_str, 0, &res),
  ...
  memory::dim query_s64(query what) const {
    memory::dim res;
    dnnl_status_t status =
        dnnl_primitive_desc_query(get(), dnnl::convert_to_c(what), 0, &res);
    return status == dnnl_success ? res : 0;
  }
  ...
  template <typename T = unsigned> T get_flags() const {
    unsigned res;
    dnnl_status_t status =
  ...
  int get_axis() const {
    int res;
    dnnl_status_t status =
        dnnl_primitive_desc_query(get(), dnnl_query_axis_s32, 0, &res);
  ...
  std::vector<float> get_factors() const {
    float *factors;
    dnnl_status_t status =
        dnnl_primitive_desc_query(get(), dnnl_query_factors, 0, &factors);
    ...
    const_dnnl_memory_desc_t md = dnnl_primitive_desc_query_md(
        get(), is_backward ? dnnl_query_diff_dst_md : dnnl_query_dst_md, 0);

    int ndims;
    error::wrap_c_api(dnnl_memory_desc_query(md, dnnl_query_ndims_s32, &ndims),
  ...
  dnnl::algorithm get_cell_kind() const { return query_alg(query::cell_kind); }
  ...
  dnnl::rnn_direction get_direction() const {
    ...
    dnnl_status_t status =
        dnnl_primitive_desc_query(get(), dnnl_query_direction, 0, &direction);
    return status == dnnl_success ? static_cast<dnnl::rnn_direction>(direction)
  ...
  dnnl::prop_kind get_prop_kind() const {
    ...
    dnnl_status_t status =
        dnnl_primitive_desc_query(get(), dnnl_query_prop_kind, 0, &prop_kind);
  ...
  memory::desc query_md(query what, int idx = 0) const {
    ...
    const_dnnl_memory_desc_t cdesc =
        dnnl_primitive_desc_query_md(get(), dnnl::convert_to_c(what), idx);
    if (!cdesc)
      return memory::desc();
  ...
  memory::desc src_desc(int idx) const { return query_md(query::src_md, idx); }
  ...
  memory::desc dst_desc(int idx) const { return query_md(query::dst_md, idx); }
  ...
  engine scratchpad_engine() const {
    ...
    error::wrap_c_api(
        dnnl_primitive_desc_query(
            get(), dnnl::convert_to_c(query::scratchpad_engine), 0, &c_engine),
  ...
  dnnl::primitive::kind get_kind() const {
    dnnl_primitive_kind_t kind;
    error::wrap_c_api(
        dnnl_primitive_desc_query(get(), dnnl_query_primitive_kind, 0,
                                  (void *)&kind),
  ...
  std::vector<uint8_t> get_cache_blob_id() const {
    ...
    error::wrap_c_api(
        dnnl_primitive_desc_query(
            get(), dnnl::convert_to_c(query::cache_blob_id), 0, (void **)&c_id),
...
protected:
  ...
  memory::dims query_dims(query what) const {
    ...
    dnnl_dims_t *c_dims;
    dnnl_status_t status =
        dnnl_primitive_desc_query(get(), dnnl::convert_to_c(what), 0, &c_dims);
    return status == dnnl_success
  ...
  engine query_engine(query what) const {
    dnnl_engine_t c_engine;
    error::wrap_c_api(dnnl_primitive_desc_query(get(), dnnl::convert_to_c(what),
                                                0, &c_engine),
  ...
  primitive_desc_base(dnnl_primitive_desc_t pd, dnnl::primitive::kind prim_kind,
    ...
    rc = dnnl_primitive_desc_query(pd, dnnl_query_primitive_kind, 0,
                                   (void *)&pd_kind);
    error::wrap_c_api(
    ...
    rc = dnnl_primitive_desc_query(pd, dnnl_query_prop_kind, 0,
                                   (void *)&pd_prop_kind);
    ...
    if ((rc == dnnl_unimplemented && c_prop_kind1 == dnnl_prop_kind_undef) ||
        (rc == dnnl_success &&
         (pd_prop_kind == c_prop_kind1 || pd_prop_kind == c_prop_kind2))) {
      reset_with_clone(pd);
      return;
    }
  ...
  struct primitive_desc : public primitive_desc_base {
    ...
    primitive_desc(const engine &src_engine, const memory::desc &src_md,
      ...
      dnnl_status_t status = dnnl_reorder_primitive_desc_create(
          &result, src_md.get(), src_engine.get(), dst_md.get(),
          dst_engine.get(), attr.get());
      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a primitive descriptor for a reorder "
                    "primitive");
    ...
    primitive_desc(const memory &src, const memory &dst,
      ...
      dnnl_status_t status = dnnl_reorder_primitive_desc_create(
          &result, src_md.get(), src.get_engine().get(), dst_md.get(),
...
/// @cond DO_NOT_DOCUMENT_THIS
inline std::vector<const_dnnl_memory_desc_t>
convert_to_c(const std::vector<memory::desc> &mds) {
...
struct concat : public primitive {
  ...
  struct primitive_desc : public primitive_desc_base {
    ...
    primitive_desc(const engine &aengine, const memory::desc &dst,
      ...
      dnnl_primitive_desc_t result;
      dnnl_status_t status = dnnl_concat_primitive_desc_create(
          &result, aengine.get(), dst.get(), (int)c_srcs.size(),
          concat_dimension, c_srcs.data(), attr.get());
      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a primitive descriptor for a concat "
                    "primitive");
      reset(status == dnnl_success ? result : dnnl_primitive_desc_t());
    }
    ...
    primitive_desc(const engine &aengine, int concat_dimension,
      ...
      dnnl_primitive_desc_t result;
      dnnl_status_t status = dnnl_concat_primitive_desc_create(
          &result, aengine.get(), nullptr, (int)c_api_srcs.size(),
...
struct sum : public primitive {
  ...
  struct primitive_desc : public primitive_desc_base {
    ...
    primitive_desc(const engine &aengine, const memory::desc &dst,
      ...
      dnnl_primitive_desc_t result;
      dnnl_status_t status = dnnl_sum_primitive_desc_create(
          &result, aengine.get(), dst.get(), (int)c_api_srcs.size(),
...
struct convolution_forward : public primitive {
  ...
  private:
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status = dnnl_convolution_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          convert_to_c(aalgorithm), src_desc.get(), weights_desc.get(),
          optional_arg(bias_desc), dst_desc.get(), &strides[0],
          optional_arg(dilates), &padding_l[0], &padding_r[0], attr.get());
      if (!allow_empty)
        error::wrap_c_api(status,
    ...
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_status_t status =
          dnnl_convolution_backward_data_primitive_desc_create(
              &pd, aengine.get(), convert_to_c(aalgorithm), diff_src_desc.get(),
              weights_desc.get(), diff_dst_desc.get(), &strides[0],
...
struct convolution_backward_weights : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    /// @returns A zero memory descriptor of the primitive does not have a
    ///          diff bias parameter.
    memory::desc diff_bias_desc() const { return base::diff_weights_desc(1); }
    ...
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_status_t status =
          dnnl_convolution_backward_weights_primitive_desc_create(
              &pd, aengine.get(), convert_to_c(aalgorithm), src_desc.get(),
              diff_weights_desc.get(), optional_arg(diff_bias_desc),
              diff_dst_desc.get(), &strides[0], optional_arg(dilates),
              &padding_l[0], &padding_r[0], hint_fwd_pd.get(), attr.get());
      if (!allow_empty)
        error::wrap_c_api(status,
                          "could not create a primitive descriptor for a "
...
struct deconvolution_backward_weights : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    /// @copydoc
    /// dnnl::convolution_backward_weights::primitive_desc::diff_bias_desc()const
    memory::desc diff_bias_desc() const { return base::diff_weights_desc(1); }
...
struct lrn_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status = dnnl_lrn_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          convert_to_c(aalgorithm), src_desc.get(), dst_desc.get(), local_size,
  ...
  ///     primitive.
  /// @param cache_blob Cache blob.
  lrn_forward(const primitive_desc &pd, const std::vector<uint8_t> &cache_blob)
      : primitive(pd, cache_blob) {}
};
...
struct lrn_backward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_status_t status = dnnl_lrn_backward_primitive_desc_create(
          &pd, aengine.get(), convert_to_c(aalgorithm), diff_src_desc.get(),
          diff_dst_desc.get(), src_desc.get(), local_size, alpha, beta, k,
          hint_fwd_pd.get(), attr.get());
  ...
  ///     primitive.
  /// @param cache_blob Cache blob.
  lrn_backward(const primitive_desc &pd, const std::vector<uint8_t> &cache_blob)
      : primitive(pd, cache_blob) {}
};
...
struct eltwise_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
                   ...
                   const primitive_attr &attr = default_attr(),
                   bool allow_empty = false)
        : primitive_desc(aengine, aprop_kind, aalgorithm, src_desc, dst_desc,
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
                   ...
                   bool allow_empty = false)
        : primitive_desc(aengine, aprop_kind, aalgorithm, src_desc, dst_desc,
                         &alpha, nullptr, attr, allow_empty) {}
  ...
  private:
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status = dnnl_eltwise_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(aalgorithm), src_desc.get(), dst_desc.get(),
  ...
  private:
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_status_t status = dnnl_eltwise_backward_primitive_desc_create(
          ...
          alpha ? *alpha : 0.0f, beta ? *beta : 0.0f, hint_fwd_pd.get(),
          attr.get());
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status = dnnl_softmax_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(aalgorithm), src_desc.get(), dst_desc.get(), axis,
          attr.get());

      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a primitive descriptor for a softmax "
                    "forward propagation primitive");
      reset(pd);
    }
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_status_t status = dnnl_softmax_backward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aalgorithm),
          diff_src_desc.get(), diff_dst_desc.get(), dst_desc.get(), axis,
          hint_fwd_pd.get(), attr.get());
...
struct batch_normalization_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status =
          dnnl_batch_normalization_forward_primitive_desc_create(
              &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
              src_desc.get(), dst_desc.get(), epsilon, convert_to_c(flags),
              attr.get());
    ...
    ///     forward propagation primitive.
    primitive_desc(dnnl_primitive_desc_t pd)
        : dnnl::primitive_desc(pd, dnnl::primitive::kind::batch_normalization,
                               dnnl::prop_kind::forward_training,
                               dnnl::prop_kind::forward_inference) {}
  ...
  private:
    ...
    memory::desc stat_desc(int kind) const {
      ...
      return query_md(use_global_stats ? query::src_md : query::dst_md, kind);
    }
  };
...
struct batch_normalization_backward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    ///     defaults to false.
    primitive_desc(
        const engine &aengine, prop_kind aprop_kind,
        const memory::desc &diff_src_desc, const memory::desc &diff_dst_desc,
        const memory::desc &src_desc, float epsilon, normalization_flags flags,
      ...
      dnnl_status_t status =
          dnnl_batch_normalization_backward_primitive_desc_create(
              &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
              diff_src_desc.get(), diff_dst_desc.get(), src_desc.get(), epsilon,
    ...
    /// @copydoc
    /// dnnl::batch_normalization_forward::primitive_desc::mean_desc()const
    memory::desc mean_desc() const { return query_md(query::src_md, 1); }

    /// @copydoc
    /// dnnl::batch_normalization_forward::primitive_desc::variance_desc()const
    memory::desc variance_desc() const { return query_md(query::src_md, 2); }
...
struct group_normalization_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    ///     forward propagation primitive.
    primitive_desc(dnnl_primitive_desc_t pd)
        : dnnl::primitive_desc(pd, dnnl::primitive::kind::group_normalization,
                               dnnl::prop_kind::forward_training,
                               dnnl::prop_kind::forward_inference) {}
  ...
  ///     propagation primitive.
  /// @param cache_blob Cache blob.
  group_normalization_forward(const primitive_desc &pd,
                              const std::vector<uint8_t> &cache_blob)
      : primitive(pd, cache_blob) {}
};
...
struct group_normalization_backward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(
        const engine &aengine, prop_kind aprop_kind,
        const memory::desc &diff_src_desc, const memory::desc &diff_dst_desc,
        const memory::desc &src_desc, memory::dim groups, float epsilon,
      ...
      dnnl_status_t status =
          dnnl_group_normalization_backward_primitive_desc_create(
              &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
              diff_src_desc.get(), diff_dst_desc.get(), src_desc.get(), groups,
              epsilon, convert_to_c(flags), hint_fwd_pd.get(), attr.get());
    ...
    /// @copydoc
    /// dnnl::group_normalization_forward::primitive_desc::mean_desc()const
    memory::desc mean_desc() const { return query_md(query::src_md, 1); }

    /// @copydoc
    /// dnnl::group_normalization_forward::primitive_desc::variance_desc()const
    memory::desc variance_desc() const { return query_md(query::src_md, 2); }
  ...
  ///     propagation primitive.
  /// @param cache_blob Cache blob.
  group_normalization_backward(const primitive_desc &pd,
                               const std::vector<uint8_t> &cache_blob)
      : primitive(pd, cache_blob) {}
};
...
struct layer_normalization_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
                   ...
                   const primitive_attr &attr = default_attr(),
                   bool allow_empty = false)
        : primitive_desc(aengine, aprop_kind, src_desc, dst_desc, &stat_desc,
    ...
    ///     forward propagation primitive.
    primitive_desc(dnnl_primitive_desc_t pd)
        : dnnl::primitive_desc(pd, dnnl::primitive::kind::layer_normalization,
                               dnnl::prop_kind::forward_training,
                               dnnl::prop_kind::forward_inference) {}
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status =
          dnnl_layer_normalization_forward_primitive_desc_create_v2(
              &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
              src_desc.get(), dst_desc.get(), optional_arg(stat_desc),
              memory::convert_to_c(scale_shift_data_type), epsilon,
              convert_to_c(flags), attr.get());
...
struct layer_normalization_backward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(
        const engine &aengine, prop_kind aprop_kind,
        const memory::desc &diff_src_desc, const memory::desc &diff_dst_desc,
        const memory::desc &src_desc, const memory::desc &stat_desc,
        float epsilon, normalization_flags flags,
        ...
        : primitive_desc(aengine, aprop_kind, diff_src_desc, diff_dst_desc,
                         ...
                         memory::data_type::f32, epsilon, flags, hint_fwd_pd,
                         attr, allow_empty) {}
    ...
    primitive_desc(
        ...
        : primitive_desc(aengine, aprop_kind, diff_src_desc, diff_dst_desc,
                         ...
                         scale_shift_data_type, epsilon, flags, hint_fwd_pd,
                         attr, allow_empty) {}
...
struct inner_product_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
                   ...
                   const primitive_attr &attr = default_attr(),
                   bool allow_empty = false)
        : primitive_desc(aengine, aprop_kind, src_desc, weights_desc, nullptr,
  ...
  private:
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_primitive_desc_t pd = nullptr;
      dnnl_status_t status = dnnl_inner_product_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind), src_desc.get(),
          weights_desc.get(), optional_arg(bias_desc), dst_desc.get(),
          attr.get());

      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a primitive descriptor for an inner "
    ...
    primitive_desc(const engine &aengine, const memory::desc &diff_src_desc,
      ...
      dnnl_status_t status =
          dnnl_inner_product_backward_data_primitive_desc_create(
              &pd, aengine.get(), diff_src_desc.get(), weights_desc.get(),
    ...
    primitive_desc(const engine &aengine, const memory::desc &src_desc,
      ...
      dnnl_status_t status =
          dnnl_inner_product_backward_weights_primitive_desc_create(
              &pd, aengine.get(), src_desc.get(), diff_weights_desc.get(),
...
struct rnn_primitive_desc_base : public primitive_desc {
  ...
  rnn_primitive_desc_base(dnnl_primitive_desc_t pd, dnnl::prop_kind aprop_kind,
  ...
  memory::desc diff_augru_attention_desc() const {
    return base::query_md(query::exec_arg_md, DNNL_ARG_DIFF_AUGRU_ATTENTION);
  }
  ...
  memory::desc diff_weights_peephole_desc() const {
    return base::query_md(query::exec_arg_md, DNNL_ARG_DIFF_WEIGHTS_PEEPHOLE);
  }
...
protected:
  ...
  // @param cell_kind Expected cell kind.
  rnn_primitive_desc_base(dnnl_primitive_desc_t pd, dnnl::prop_kind prop_kind1,
    ...
    rc = dnnl_primitive_desc_query(pd, dnnl_query_prop_kind, 0, &q_prop_kind);
    error::wrap_c_api(rc,
                      "could not retrieve a propagation kind from a primitive "
                      "descriptor for an RNN primitive");

    dnnl_alg_kind_t q_cell_kind;
    rc = dnnl_primitive_desc_query(pd, dnnl_query_cell_kind, 0, &q_cell_kind);
    error::wrap_c_api(
        rc, "could not retrieve a cell kind from a primitive descriptor "
  ...
  rnn_primitive_desc_base(
      const engine &aengine, algorithm cell_kind, prop_kind aprop_kind,
      algorithm activation, rnn_direction direction,
      const memory::desc &src_layer_desc, const memory::desc &src_iter_desc,
      const memory::desc *src_iter_c_desc, const memory::desc *attention_desc,
      ...
      const memory::desc &dst_iter_desc, const memory::desc *dst_iter_c_desc,
      rnn_flags flags, float alpha, float beta, const primitive_attr &attr,
      bool allow_empty) {

    dnnl_status_t status = dnnl_success;
    const char *msg = "could not create a primitive descriptor for a requested "
                      "cell kind";

    dnnl_primitive_desc_t pd = nullptr;
    switch (cell_kind) {
    case algorithm::vanilla_rnn:
      status = dnnl_vanilla_rnn_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(activation), dnnl::convert_to_c(direction),
          src_layer_desc.get(), src_iter_desc.get(), weights_layer_desc.get(),
          weights_iter_desc.get(), bias_desc.get(), dst_layer_desc.get(),
          dst_iter_desc.get(), convert_to_c(flags), alpha, beta, attr.get());
      msg = "could not create a primitive descriptor for a vanilla "
            "RNN forward propagation primitive";
      break;
    case algorithm::vanilla_lstm:
      status = dnnl_lstm_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          ...
          optional_arg(dst_iter_c_desc), convert_to_c(flags), attr.get());
      msg = "could not create a primitive descriptor for an LSTM "
            "forward propagation primitive";
      break;
    case algorithm::vanilla_gru:
      status = dnnl_gru_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
    ...
    case algorithm::lbr_gru:
      status = dnnl_lbr_gru_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), weights_layer_desc.get(),
          weights_iter_desc.get(), bias_desc.get(), dst_layer_desc.get(),
    ...
    case algorithm::vanilla_augru:
      status = dnnl_augru_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), optional_arg(attention_desc),
          weights_layer_desc.get(), weights_iter_desc.get(), bias_desc.get(),
          dst_layer_desc.get(), dst_iter_desc.get(), convert_to_c(flags),
          attr.get());
      msg = "could not create a primitive descriptor for an AUGRU "
            "forward propagation primitive";
      break;
    case algorithm::lbr_augru:
      status = dnnl_lbr_augru_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), optional_arg(attention_desc),
          weights_layer_desc.get(), weights_iter_desc.get(), bias_desc.get(),
      ...
      break;
    default:
      status = dnnl_unimplemented;
    }

    if (!allow_empty)
      error::wrap_c_api(status, msg);
    reset(pd);
  }
  ...
  rnn_primitive_desc_base(
      ...
      const memory::desc *diff_dst_iter_c_desc, rnn_flags flags, float alpha,
      float beta, const rnn_primitive_desc_base &hint_fwd_pd,
      const primitive_attr &attr, bool allow_empty) {
    ...
    switch (cell_kind) {
    case algorithm::vanilla_rnn:
      status = dnnl_vanilla_rnn_backward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(activation), dnnl::convert_to_c(direction),
          src_layer_desc.get(), src_iter_desc.get(), weights_layer_desc.get(),
          weights_iter_desc.get(), bias_desc.get(), dst_layer_desc.get(),
          dst_iter_desc.get(), diff_src_layer_desc.get(),
          diff_src_iter_desc.get(), diff_weights_layer_desc.get(),
          diff_weights_iter_desc.get(), diff_bias_desc.get(),
          diff_dst_layer_desc.get(), diff_dst_iter_desc.get(),
          convert_to_c(flags), alpha, beta, hint_fwd_pd.get(), attr.get());
      msg = "could not create a primitive descriptor for a vanilla "
            "RNN backward propagation primitive";
      break;
    case algorithm::vanilla_lstm:
      status = dnnl_lstm_backward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), optional_arg(src_iter_c_desc),
          weights_layer_desc.get(), weights_iter_desc.get(),
          ...
          optional_arg(weights_projection_desc), bias_desc.get(),
          dst_layer_desc.get(), dst_iter_desc.get(),
          optional_arg(dst_iter_c_desc), diff_src_layer_desc.get(),
          diff_src_iter_desc.get(), optional_arg(diff_src_iter_c_desc),
          diff_weights_layer_desc.get(), diff_weights_iter_desc.get(),
          optional_arg(diff_weights_peephole_desc),
          optional_arg(diff_weights_projection_desc), diff_bias_desc.get(),
          diff_dst_layer_desc.get(), diff_dst_iter_desc.get(),
      ...
      break;
    case algorithm::vanilla_gru:
      status = dnnl_gru_backward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), weights_layer_desc.get(),
          weights_iter_desc.get(), bias_desc.get(), dst_layer_desc.get(),
      ...
      break;
    case algorithm::lbr_gru:
      status = dnnl_lbr_gru_backward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), weights_layer_desc.get(),
          weights_iter_desc.get(), bias_desc.get(), dst_layer_desc.get(),
      ...
      break;
    case algorithm::vanilla_augru:
      status = dnnl_augru_backward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), optional_arg(attention_desc),
          weights_layer_desc.get(), weights_iter_desc.get(), bias_desc.get(),
          dst_layer_desc.get(), dst_iter_desc.get(), diff_src_layer_desc.get(),
          diff_src_iter_desc.get(), optional_arg(diff_attention_desc),
          diff_weights_layer_desc.get(), diff_weights_iter_desc.get(),
          diff_bias_desc.get(), diff_dst_layer_desc.get(),
          diff_dst_iter_desc.get(), convert_to_c(flags), hint_fwd_pd.get(),
          attr.get());
      msg = "could not create a primitive descriptor for an AUGRU "
            "backward propagation primitive";
      break;
    case algorithm::lbr_augru:
      status = dnnl_lbr_augru_backward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          dnnl::convert_to_c(direction), src_layer_desc.get(),
          src_iter_desc.get(), optional_arg(attention_desc),
          weights_layer_desc.get(), weights_iter_desc.get(), bias_desc.get(),
...
struct vanilla_rnn_forward : public primitive {
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    ///     defaults to false.
    primitive_desc(
        const engine &aengine, prop_kind aprop_kind, algorithm activation,
        rnn_direction direction, const memory::desc &src_layer_desc,
        const memory::desc &src_iter_desc,
        const memory::desc &weights_layer_desc,
        const memory::desc &weights_iter_desc, const memory::desc &bias_desc,
        const memory::desc &dst_layer_desc, const memory::desc &dst_iter_desc,
        const primitive_attr &attr = default_attr(), bool allow_empty = false)
        : rnn_primitive_desc_base(
              aengine, algorithm::vanilla_rnn, aprop_kind, activation,
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
        ...
        : rnn_primitive_desc_base(
              ...
              weights_layer_desc, weights_iter_desc, nullptr, nullptr,
              bias_desc, dst_layer_desc, dst_iter_desc, nullptr,
              rnn_flags::undef, alpha, 0.0f, attr, allow_empty) {}
    ...
    memory::desc src_layer_desc() const { return rnn_base::src_layer_desc(); }
    ...
    memory::desc dst_layer_desc() const { return rnn_base::dst_layer_desc(); }
    ...
    memory::desc workspace_desc() const { return rnn_base::workspace_desc(); }
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(
        ...
        : rnn_primitive_desc_base(
              ...
              rnn_flags::undef, 0.0f, 0.0f, hint_fwd_pd, attr, allow_empty) {}
    ...
    primitive_desc(
        ...
        : rnn_primitive_desc_base(
              ...
              diff_weights_layer_desc, diff_weights_iter_desc, nullptr, nullptr,
              diff_bias_desc, diff_dst_layer_desc, diff_dst_iter_desc, nullptr,
              rnn_flags::undef, alpha, 0.0f, hint_fwd_pd, attr, allow_empty) {}
    ...
    memory::desc diff_bias_desc() const { return rnn_base::diff_bias_desc(); }
...
struct lstm_forward : public primitive {
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    ///     defaults to false.
    primitive_desc(
        const engine &aengine, prop_kind aprop_kind, rnn_direction direction,
        const memory::desc &src_layer_desc, const memory::desc &src_iter_desc,
        ...
        const primitive_attr &attr = default_attr(), bool allow_empty = false)
        : rnn_primitive_desc_base(
              aengine, algorithm::vanilla_lstm, aprop_kind, algorithm::undef,
              direction, src_layer_desc, src_iter_desc, &src_iter_c_desc,
              nullptr, weights_layer_desc, weights_iter_desc,
              &weights_peephole_desc, &weights_projection_desc, bias_desc,
              dst_layer_desc, dst_iter_desc, &dst_iter_c_desc, rnn_flags::undef,
    ...
    primitive_desc(
        ...
        : rnn_primitive_desc_base(
              ...
              dst_iter_desc, &dst_iter_c_desc, rnn_flags::undef, 0.0f, 0.0f,
              attr, allow_empty) {}
    ...
    /// @copydoc dnnl::rnn_primitive_desc_base::src_iter_desc()const
    memory::desc src_iter_c_desc() const { return rnn_base::src_iter_c_desc(); }
    ...
    /// @copydoc dnnl::rnn_primitive_desc_base::src_iter_desc()const
    memory::desc dst_iter_c_desc() const { return rnn_base::dst_iter_c_desc(); }
  ...
  ///     primitive.
  /// @param cache_blob Cache blob.
  lstm_forward(const primitive_desc &pd, const std::vector<uint8_t> &cache_blob)
      : primitive(pd, cache_blob) {}
};
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(
        ...
        : rnn_primitive_desc_base(
              ...
              &diff_dst_iter_c_desc, rnn_flags::undef, 0.0f, 0.0f, hint_fwd_pd,
              attr, allow_empty) {}
    ...
    primitive_desc(
        ...
        : rnn_primitive_desc_base(
              ...
              bias_desc, dst_layer_desc, dst_iter_desc, &dst_iter_c_desc,
              diff_src_layer_desc, diff_src_iter_desc, &diff_src_iter_c_desc,
              nullptr, diff_weights_layer_desc, diff_weights_iter_desc, nullptr,
              nullptr, diff_bias_desc, diff_dst_layer_desc, diff_dst_iter_desc,
              &diff_dst_iter_c_desc, rnn_flags::undef, 0.0f, 0.0f, hint_fwd_pd,
              attr, allow_empty) {}
    ...
    /// @copydoc
    /// dnnl::rnn_primitive_desc_base::diff_weights_peephole_desc()const
    memory::desc diff_weights_peephole_desc() const {
      return rnn_base::diff_weights_peephole_desc();
    }

    /// @copydoc
    /// dnnl::rnn_primitive_desc_base::diff_weights_projection_desc()const
    memory::desc diff_weights_projection_desc() const {
...
struct gru_forward : public primitive {
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(
        ...
        const primitive_attr &attr = default_attr(), bool allow_empty = false)
        : rnn_primitive_desc_base(
              aengine, algorithm::vanilla_gru, aprop_kind, algorithm::undef,
              direction, src_layer_desc, src_iter_desc, nullptr, nullptr,
              weights_layer_desc, weights_iter_desc, nullptr, nullptr,
              bias_desc, dst_layer_desc, dst_iter_desc, nullptr,
              rnn_flags::undef, 0.0f, 0.0f, attr, allow_empty) {}
  ...
  ///     primitive.
  /// @param cache_blob Cache blob.
  gru_forward(const primitive_desc &pd, const std::vector<uint8_t> &cache_blob)
      : primitive(pd, cache_blob) {}
};
...
struct gru_backward : public primitive {
  ...
  ///     primitive.
  /// @param cache_blob Cache blob.
  gru_backward(const primitive_desc &pd, const std::vector<uint8_t> &cache_blob)
      : primitive(pd, cache_blob) {}
};
...
struct lbr_gru_forward : public primitive {
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(
        ...
        const primitive_attr &attr = default_attr(), bool allow_empty = false)
        : rnn_primitive_desc_base(
              aengine, algorithm::lbr_gru, aprop_kind, algorithm::undef,
              direction, src_layer_desc, src_iter_desc, nullptr, nullptr,
              weights_layer_desc, weights_iter_desc, nullptr, nullptr,
              bias_desc, dst_layer_desc, dst_iter_desc, nullptr,
              rnn_flags::undef, 0.0f, 0.0f, attr, allow_empty) {}
...
struct augru_forward : public primitive {
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(
        ...
        const primitive_attr &attr = default_attr(), bool allow_empty = false)
        : rnn_primitive_desc_base(
              aengine, algorithm::vanilla_augru, aprop_kind, algorithm::undef,
              direction, src_layer_desc, src_iter_desc, nullptr,
              &attention_desc, weights_layer_desc, weights_iter_desc, nullptr,
              nullptr, bias_desc, dst_layer_desc, dst_iter_desc, nullptr,
              rnn_flags::undef, 0.0f, 0.0f, attr, allow_empty) {}
...
struct lbr_augru_forward : public primitive {
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(
        ...
        const primitive_attr &attr = default_attr(), bool allow_empty = false)
        : rnn_primitive_desc_base(
              aengine, algorithm::lbr_augru, aprop_kind, algorithm::undef,
              direction, src_layer_desc, src_iter_desc, nullptr,
              &attention_desc, weights_layer_desc, weights_iter_desc, nullptr,
              nullptr, bias_desc, dst_layer_desc, dst_iter_desc, nullptr,
              rnn_flags::undef, 0.0f, 0.0f, attr, allow_empty) {}
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status = dnnl_shuffle_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind), src_desc.get(),
          dst_desc.get(), axis, group_size, attr.get());

      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a primitive descriptor for a shuffle "
                    "forward propagation primitive");
      reset(pd);
    }
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, const memory::desc &diff_src_desc,
      ...
      dnnl_status_t status = dnnl_shuffle_backward_primitive_desc_create(
          &pd, aengine.get(), diff_src_desc.get(), diff_dst_desc.get(), axis,
          group_size, hint_fwd_pd.get(), attr.get());
...
struct binary : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_primitive_desc_t pd = nullptr;
      dnnl_status_t status = dnnl_binary_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aalgorithm), src0.get(),
          src1.get(), dst.get(), attr.get());

      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a primitive descriptor for a binary "
                    "operation primitive");
      reset(pd);
    }
...
struct matmul : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, const memory::desc &src_desc,
                   ...
                   const primitive_attr &attr = default_attr(),
                   bool allow_empty = false)
        : primitive_desc(aengine, src_desc, weights_desc, &bias_desc, dst_desc,
                         attr, allow_empty) {}
    ...
    /// @copydoc dnnl::primitive_desc_base::weights_desc()const
    memory::desc weights_desc() const { return query_md(query::weights_md, 0); }

    /// @copydoc dnnl::convolution_forward::primitive_desc::bias_desc()const
    memory::desc bias_desc() const { return query_md(query::weights_md, 1); }
  ...
  private:
    primitive_desc(const engine &aengine, const memory::desc &src_desc,
      ...
      dnnl_primitive_desc_t pd = nullptr;
      dnnl_status_t status = dnnl_matmul_primitive_desc_create(
          &pd, aengine.get(), src_desc.get(), weights_desc.get(),
          optional_arg(bias_desc), dst_desc.get(), attr.get());

      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a primitive descriptor for a matmul "
                    "primitive");
      reset(pd);
    }
  };
...
struct resampling_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
                   ...
                   const primitive_attr &attr = default_attr(),
                   bool allow_empty = false)
        : primitive_desc(aengine, aprop_kind, aalgorithm, &factors, src_desc,
  ...
  private:
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_primitive_desc_t pd = nullptr;
      dnnl_status_t status = dnnl_resampling_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          convert_to_c(aalgorithm), optional_arg(factors), src_desc.get(),
          optional_arg(dst_desc), attr.get());
...
struct resampling_backward : public primitive {
  ...
  private:
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_primitive_desc_t pd = nullptr;
      dnnl_status_t status = dnnl_resampling_backward_primitive_desc_create(
          &pd, aengine.get(), convert_to_c(aalgorithm), optional_arg(factors),
          diff_src_desc.get(), diff_dst_desc.get(), hint_fwd_pd.get(),
          attr.get());
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_status_t status = dnnl_pooling_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind),
          convert_to_c(aalgorithm), src_desc.get(), dst_desc.get(), &strides[0],
          &kernel[0], &dilation[0], &padding_l[0], &padding_r[0], attr.get());
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_status_t status = dnnl_pooling_backward_primitive_desc_create(
          &pd, aengine.get(), convert_to_c(aalgorithm), diff_src_desc.get(),
          diff_dst_desc.get(), &strides[0], &kernel[0], &dilation[0],
          &padding_l[0], &padding_r[0], hint_fwd_pd.get(), attr.get());
      if (!allow_empty)
        error::wrap_c_api(
            status, "could not create a descriptor for a pooling backward "
                    "propagation primitive");
      reset(pd);
    }
...
struct prelu_forward : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, prop_kind aprop_kind,
      ...
      dnnl_primitive_desc_t pd = nullptr;
      dnnl_status_t status = dnnl_prelu_forward_primitive_desc_create(
          &pd, aengine.get(), dnnl::convert_to_c(aprop_kind), src_desc.get(),
          weight_desc.get(), dst_desc.get(), attr.get());
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, const memory::desc &src_desc,
      ...
      dnnl_status_t status = dnnl_prelu_backward_primitive_desc_create(
          ...
          diff_src_desc.get(), diff_weights_desc.get(), diff_dst_desc.get(),
          hint_fwd_pd.get(), attr.get());
...
struct reduction : public primitive {
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const engine &aengine, algorithm aalgorithm,
      ...
      dnnl_primitive_desc_t pd = nullptr;
      dnnl_status_t status = dnnl_reduction_primitive_desc_create(
          &pd, aengine.get(), convert_to_c(aalgorithm), src_desc.get(),
...
inline const version_t *version() { return dnnl_version(); }
...
inline status set_default_fpmath_mode(fpmath_mode mode) {
  return static_cast<status>(dnnl_set_default_fpmath_mode(convert_to_c(mode)));
}
...
inline status set_cpu_isa_hints(cpu_isa_hints isa_hints) {
  return static_cast<status>(
      dnnl_set_cpu_isa_hints(static_cast<dnnl_cpu_isa_hints_t>(isa_hints)));
}
...
inline void reset_profiling(stream &stream) {
  error::wrap_c_api(dnnl_reset_profiling(stream.get()),
                    "could not reset profiling");
}
...
inline std::vector<uint64_t> get_profiling_data(stream &stream,
                                                profiling_data_kind data_kind) {
  int num_entries = 0;
  error::wrap_c_api(dnnl_query_profiling_data(
                        stream.get(),
...
inline status sgemm(char transa, char transb, dnnl_dim_t M, dnnl_dim_t N,
                    ...
                    const float *B, dnnl_dim_t ldb, float beta, float *C,
                    dnnl_dim_t ldc) {
  return static_cast<status>(
...
inline status gemm_u8s8s32(char transa, char transb, char offsetc, dnnl_dim_t M,
                           ...
                           const uint8_t *A, dnnl_dim_t lda, uint8_t ao,
                           const int8_t *B, dnnl_dim_t ldb, int8_t bo,
                           float beta, int32_t *C, dnnl_dim_t ldc,
                           const int32_t *co) {
  return static_cast<status>(dnnl_gemm_u8s8s32(transa, transb, offsetc, M, N, K,
                                               alpha, A, lda, ao, B, ldb, bo,
                                               beta, C, ldc, co));
}
...
inline status gemm_s8s8s32(char transa, char transb, char offsetc, dnnl_dim_t M,
                           dnnl_dim_t N, dnnl_dim_t K, float alpha,
                           const int8_t *A, dnnl_dim_t lda, int8_t ao,
                           const int8_t *B, dnnl_dim_t ldb, int8_t bo,
                           float beta, int32_t *C, dnnl_dim_t ldc,
                           const int32_t *co) {
  return static_cast<status>(dnnl_gemm_s8s8s32(transa, transb, offsetc, M, N, K,
                                               alpha, A, lda, ao, B, ldb, bo,
                                               beta, C, ldc, co));
}
...
inline primitive::primitive(const_dnnl_primitive_desc_t c_pd,
  ...
  error::wrap_c_api(dnnl_primitive_create_from_cache_blob(&result, c_pd, size,
                                                          cache_blob_data),
...
inline primitive::primitive(const primitive_desc &pd,
                            const std::vector<uint8_t> &cache_blob)
    : primitive(pd.get(), cache_blob) {}

inline void
primitive::execute(const stream &astream,
                   const std::unordered_map<int, memory> &args) const {
